...
public class P2PBridgeManager {
    ...
    public interface Listener {
        ...
        /** 桌面端 UPnP 已开公网端口，可直接直连（免 WebRTC） */
        default void onDirectConnect(String publicIp, int port) {}
    }
    ...
    public class Bridge {
        ...
        @JavascriptInterface
        public void onPublicIp(String publicIp, int port) {
            Log.d(TAG, "桌面端 UPnP 直连可用: " + publicIp + ":" + port);
            if (listener != null) listener.onDirectConnect(publicIp, port);
        }
    }
}