...
public class MDNSDiscover {
    ...
    public interface DiscoveryListener {
        void onDesktopFound(String host, int port, String name, String token);
    ...
    public void startDiscovery() {
        ...
        resolveListener = new NsdManager.ResolveListener() {
            ...
            public void onServiceResolved(NsdServiceInfo serviceInfo) {
                ...
                // TXT 记录里读访问令牌（桌面端 mDNS 广播 token=xxx）
                String token = "";
                try {
                    java.util.Map<String, byte[]> attrs = serviceInfo.getAttributes();
                    byte[] t = attrs != null ? attrs.get("token") : null;
                    if (t != null) token = new String(t, "UTF-8");
                } catch (Exception ignored) {}
                Log.d(TAG, "发现: " + name + " at " + host + ":" + port + " token=" + (token.isEmpty() ? "(无)" : "有"));
                if (listener != null) {
                    listener.onDesktopFound(host, port, name, token);
                }
            }
        };
